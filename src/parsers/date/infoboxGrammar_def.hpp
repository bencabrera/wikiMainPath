...
namespace WikiMainPath {
	...
	template<typename Iterator, typename Skipper>
		InfoboxGrammar<Iterator, Skipper>::InfoboxGrammar() : InfoboxGrammar::base_type(article, "InfoboxGrammar") {
			...
			start_infobox = lit("{{") >> no_case["infobox"] >> *(char_ - eol) >> eol;
			infobox = start_infobox >> ((&infobox_line_with_date >> infobox_line_with_date [boost::phoenix::push_back(_val, boost::spirit::_1)])| *(char_ - eol) ) % (eol >> !lit("}}"))
						>> -eol >> lit("}}");

			infobox_label = *(char_ - '=') [_val += boost::spirit::_1];
			infobox_line_with_date = lit('|') >> infobox_label [at_c<0>(_val) = boost::spirit::_1] >> '=' >> date_string [at_c<1>(_val) = boost::spirit::_1];
